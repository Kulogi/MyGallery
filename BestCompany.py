...
class_list = ("Button", "CheckBox", "Chronometer", "CompoundButton", "ImageButton", "ProgressBar",
...
company_list = {"Microsoft Corporation", "Google Inc.", "AI Factory Limited", "GIOSIS PTE. LTD.","JRJ Unlimited","SMSROBOT LTD", "NAVER Corp."
           ,"Corcanoe", "Gemego Ltd", "SMSROBOT LTD", "Mirco Bettelini", "Boreumdal_Lab", "Code Sector", "RATP", "joysoft", "Molatra"
           ,"NyxCore", "YinzCam, Inc.", "Recruit Holdings Co.,Ltd."}

no = 0 # the unique id of every screenshot
...
def get_ocrop(imgsize, src_temp, pic):
    ...
    region.save("./Mynewscreenshot/{}-{}.png".format(src_temp, pic))
...
""" function list end """
json_file = open("company.json", 'w')
json_zone = open("company_backup.json", 'w')
file = open("meta.json")
meta = json.load(file)
...
for root, dirs, files in os.walk(rootdir):
    ...
    for file in files:
        ...
        if tired >= tired_control: # to control too many loops
            tired = 0
            break
        ...
        if os.path.splitext(file)[1] == '.xml':
            ...
            for n in Nodes:
                ...
                if access_time == 0:
                    ...
                    if developer not in company_list:
                        print("wrong developers : ",developer," file resource : ", xmlfile)
                        flag = 1
                        break
                ...
                if classname in class_list  and index != -1: #in class_list:          
                    ...
                    if no >= 1  and ratio < ratio_control and (w,h) not in dic.values():
                        tired += 1 #get tired
